...
export class UserDomainService {
  ...
  /**
   * Determines if a user can be promoted to tutor based on business rules
   */
  canUserBePromotedToTutor(user: User, requirements: TutorRequirements): boolean {
    this.logger.debug(`Checking tutor promotion eligibility for user ${user.id.value}`);
    ...
    // Must not already be a tutor or admin
    if (user.role.isTutor() || user.role.isAdmin()) {
      this.logger.debug('User already has elevated role');
      return false;
    }

    // Check registration time
    const daysSinceRegistration = this.calculateDaysSinceRegistration(user);
    if (daysSinceRegistration < requirements.minRegistrationDays) {
      this.logger.debug(
        `User registered only ${daysSinceRegistration} days ago, requires ${requirements.minRegistrationDays}`
      );
      return false;
    }

    this.logger.debug('User meets tutor promotion requirements');
    return true;
  }
  ...
  validateAndLogRoleTransition(from: UserRole, to: UserRole, user: User, requestedBy: User): void {
    this.logger.debug(
      `Validating role transition from ${from.value} to ${to.value} for user ${user.id.value} requested by ${requestedBy.id.value}`
    );
    ...
    if (!UserBusinessRules.canTransitionRole(from, to, user)) {
      ...
      throw new Error(`Role transition from ${from.value} to ${to.value} is not allowed`);
    }
    ...
    if (to.isTutor() && !requestedBy.role.canManageUsers()) {
      ...
      throw new Error('Only administrators can promote users to tutor role');
    }

    if (!requestedBy.isActive) {
      this.logger.warn(`Inactive user ${requestedBy.id.value} attempted role transition`);
      throw new Error('Inactive users cannot perform role transitions');
    }
  ...
  generateUserMetrics(
    ...
    const isEligibleForPremium = reputationScore
      ? UserBusinessRules.hasPremiumAccess(user, reputationScore)
      : false;
  ...
  private calculateDaysSinceRegistration(user: User): number {
    const now = new Date();
