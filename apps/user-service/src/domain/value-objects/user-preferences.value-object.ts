 ...
 *
 * Encapsulates user preferences including timezone, language,
 * and notification settings with business logic for preference validation
 */
export class UserPreferences {
   ...
   */
  static createDefault(timezone: string = 'UTC', language: Language = Language.ENGLISH): UserPreferences {
  ...
  /**
   * Determines if user should receive a specific notification type
   */
  shouldReceiveNotification(type: NotificationType): boolean {
    // Security alerts are always enabled (business rule)
    if (type === NotificationType.SECURITY_ALERTS) {
      this.logger.debug(`Security alerts are mandatory for all users`);
      return true;
    }

    // System emails are always enabled (business rule)
    if (type === NotificationType.EMAIL_SYSTEM) {
      this.logger.debug(`System emails are mandatory for all users`);
      return true;
    }
  ...
  updateNotificationSetting(type: NotificationType, enabled: boolean): UserPreferences {
    ...
    if ((type === NotificationType.SECURITY_ALERTS || type === NotificationType.EMAIL_SYSTEM) && !enabled) {
      this.logger.warn(`Attempted to disable mandatory notification type: ${type}`);
      throw new Error(`Cannot disable ${type} notifications - they are required for account security`);
    }
  ...
  hasMinimalNotifications(): boolean {
    ...
    return this._notificationSettings[NotificationType.EMAIL_SYSTEM] &&
  ...
  get notificationSettings(): NotificationSettings { return { ...this._notificationSettings }; }
  get theme(): 'light' | 'dark' | 'system' { return this._theme; }
  get dateFormat(): 'MM/DD/YYYY' | 'DD/MM/YYYY' | 'YYYY-MM-DD' { return this._dateFormat; }
  get timeFormat(): '12h' | '24h' { return this._timeFormat; }
  ...
  equals(other: UserPreferences): boolean {
    ...
    return this._timezone === other._timezone &&
   ...
   */
  toString(): string {
    return `UserPreferences(timezone=${this._timezone}, language=${this._language}, ` +
