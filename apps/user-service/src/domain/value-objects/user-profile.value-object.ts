 ...
 *
 * Encapsulates rich user profile information including bio, skills,
 * experience, education, and achievements with business logic
 */
export class UserProfile {
  ...
  getHighestSkillLevel(): ExperienceLevel {
    ...
    const levels = [ExperienceLevel.BEGINNER, ExperienceLevel.INTERMEDIATE, ExperienceLevel.ADVANCED, ExperienceLevel.EXPERT];
  ...
  calculateCompleteness(): number {
    ...
    if ((this._website ? 1 : 0) + (this._linkedInUrl ? 1 : 0) + (this._githubUrl ? 1 : 0) >= 2) score += 1;
  ...
  /**
   * Checks if profile is complete enough for tutoring
   */
  isCompleteForTutoring(): boolean {
    const completeness = this.calculateCompleteness();

    // Require at least 70% completeness for tutoring
    if (completeness < 70) {
      this.logger.debug(`Profile completeness ${completeness}% is below 70% threshold for tutoring`);
      return false;
    }

    // Must have bio
    if (!this._bio || this._bio.length < 50) {
      this.logger.debug('Bio must be at least 50 characters for tutoring');
      return false;
    }

    // Must have at least 3 skills
    if (this._skills.length < 3) {
      this.logger.debug('Must have at least 3 skills for tutoring');
      return false;
    }

    // Must have some education or achievements
    if (this._education.length === 0 && this._achievements.length === 0) {
      this.logger.debug('Must have education or achievements for tutoring');
      return false;
    }
  ...
  removeSkill(skillName: string): UserProfile {
    const filteredSkills = this._skills.filter(skill =>
      skill.name.toLowerCase() !== skillName.toLowerCase()
    );
  ...
  get linkedInUrl(): string | null { return this._linkedInUrl; }
  get githubUrl(): string | null { return this._githubUrl; }
  get education(): EducationEntry[] { return [...this._education]; }
  ...
  private static validateUrls(website?: string, linkedInUrl?: string, githubUrl?: string): void {
    ...
    if (linkedInUrl && (!urlRegex.test(linkedInUrl) || !linkedInUrl.includes('linkedin.com'))) {
      throw new Error('LinkedIn URL must be a valid LinkedIn profile');
    }

    if (githubUrl && (!urlRegex.test(githubUrl) || !githubUrl.includes('github.com'))) {
      throw new Error('GitHub URL must be a valid GitHub profile');
    }
  }
   ...
   */
  toString(): string {
    return `UserProfile(skills=${this._skills.length}, completeness=${this.calculateCompleteness()}%, ` +
           `level=${this._experienceLevel}, age=${this.age || 'unknown'})`;
  }
}
