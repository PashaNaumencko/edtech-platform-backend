...
describe("UserPreferences", () => {
  describe("create", () => {
    it("should create preferences with valid data", () => {
      ...
      expect(preferences.getNotificationSetting(NotificationType.EMAIL_SYSTEM)).toBe(true);
      expect(preferences.getNotificationSetting(NotificationType.EMAIL_MARKETING)).toBe(false);
    });
  ...
  describe("createDefault", () => {
    it("should create default preferences", () => {
      ...
      expect(preferences.language).toBe(Language.ENGLISH);
      expect(preferences.getNotificationSetting(NotificationType.EMAIL_SYSTEM)).toBe(true);
      expect(preferences.getNotificationSetting(NotificationType.SECURITY_ALERTS)).toBe(true);
      expect(preferences.getNotificationSetting(NotificationType.EMAIL_MARKETING)).toBe(false);
    });
  ...
  describe("getNotificationSetting", () => {
    ...
    it("should return notification setting value", () => {
      expect(preferences.getNotificationSetting(NotificationType.EMAIL_SYSTEM)).toBe(true);
      expect(preferences.getNotificationSetting(NotificationType.EMAIL_MARKETING)).toBe(false);
      expect(preferences.getNotificationSetting(NotificationType.SECURITY_ALERTS)).toBe(true);
    });
  ...
  describe("updateNotificationSetting", () => {
    ...
    it("should update notification setting", () => {
      const updatedPreferences = preferences.updateNotificationSetting(
        NotificationType.EMAIL_MARKETING,
        true,
      );

      expect(updatedPreferences.getNotificationSetting(NotificationType.EMAIL_MARKETING)).toBe(
        true,
      );
      expect(preferences.getNotificationSetting(NotificationType.EMAIL_MARKETING)).toBe(false);
    });

    it("should update any notification setting including system ones", () => {
      const updatedPreferences = preferences.updateNotificationSetting(
        NotificationType.COURSE_UPDATES,
        false,
      );

      expect(updatedPreferences.getNotificationSetting(NotificationType.COURSE_UPDATES)).toBe(
        false,
      );
      expect(preferences.getNotificationSetting(NotificationType.COURSE_UPDATES)).toBe(true);
    });
  });
